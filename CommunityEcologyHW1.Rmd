...
setwd("~/Box Sync/UCLA/Ecology 200B")
#read in data
...
plot25 <- merge(temp, species_pool, by = "species")
...
## as we have a bunch of ties here, the convention is to rank your observed value in the middle of the ties. 
## p results (note that the tradition is also to add the observed value to the null distribution, hence 999 reps in the null and 999+1 in the denominator of the p value calculation)
print(c("one tailed test, range smaller than expected, p:", (n_less2 + n_tied2/2) / (reps+1) ))
print(c("one tailed test, range greater than expected, p:", (n_more2 + n_tied2/2) / (reps+1) ))
## two tailed tests:
if(n_less2< ((reps+1)/2)){
...
```{r}
#Create null model of log leaf areas to compare with plots 20, 21, 28. Since they all have a species N = 7, you can use the same model for them all. I am giving each individual an equal chance of dispersing into the random community. 

N_stream <- length(plot20) # indicates how large a sample should be drawn for null communities

## vector to store expected ranges of log leaf area from null communities:
lla_null_range_stream <-NA

## loop to generate null communities and calculate expected log leaf area trait ranges
for(ii in 1:reps){
  
  sample(species_pool$logLeafArea, N_stream, prob = species_pool$logLeafArea/sum(species_pool$abundance))->lla_random_community_stream
  
  lla_null_range_stream[ii]<-max(lla_random_community_stream)-min(lla_random_community_stream))
  
}

hist(lla_null_range_p20, xlab= "range of log leaf area", ylab = "distribution from null model, N=7", main= "Histogram of null model range of log leaf area for plot 20", xlim = c(0,4))
# quick *estimate* of sig. thresholds for plotting (for a two-tailed test)
quantile(lla_null_range_p20, probs=c(0.025, .975))->cutoffs_p20
abline(v=cutoffs_p20, lty=2)
text("2.5%", x=cutoffs_p20[1], y=180, pos=4)
text("97.5%", x=cutoffs_p20[2], y=180, pos=4)

## expected value (mean) from null:
abline(v=mean(lla_null_range_p20), lty=2)
text("mean", x=mean(lla_null_range_p20), y=265, pos=4)

## add the observed range of log leaf area from plot 20
abs(max(plot20$logLeafArea) - min(plot20$logLeafArea))->lla_obs_range_p20
...
## how many null values is the observed value less than?
n_more_p20 <- sum(lla_obs_range_p20<lla_null_range_p20)
## how many null values is the observed value greater than?
n_less_p20 <-sum(lla_obs_range_p20>lla_null_range_p20)
## how many null values is the observed value tied with \
n_tied_p20<-sum(lla_obs_range_p20==lla_null_range_p20)
...
```{r}
#Create null model of log leaf areas for plot 21

Np21 <- length(plot21) # indicates how large a sample should be drawn for null communities

## vector to store expected ranges of log leaf area from null communities:
lla_null_range_p21<-NA

## loop to generate null communities and calculate expected log leaf area trait ranges
for(ii in 1:reps){
  
  sample(species_pool$logLeafArea, Np21)->lla_random_community_p21
  
  lla_null_range_p21[ii]<-abs(max(lla_random_community_p21)-min(lla_random_community_p21))
  
}

hist(lla_null_range_p21, xlab= "range of log leaf area", ylab = "distribution from null model, N=7", main= "Histogram of null model range of log leaf area for plot 21", xlim = c(0,4))
# quick *estimate* of sig. thresholds for plotting (for a two-tailed test)
quantile(lla_null_range_p21, probs=c(0.025, .975))->cutoffs_p21
abline(v=cutoffs_p21, lty=2)
text("2.5%", x=cutoffs_p21[1], y=180, pos=4)
text("97.5%", x=cutoffs_p21[2], y=180, pos=4)

## expected value (mean) from null:
abline(v=mean(lla_null_range_p21), lty=2)
text("mean", x=mean(lla_null_range_p21), y=265, pos=4)
...
text("observed value", x=lla_obs_range_p21, y= 260, pos=4, col="red")
...
Np28 <- length(plot28) # indicates how large a sample should be drawn for null communities

## vector to store expected ranges of log leaf area from null communities:
lla_null_range_p28<-NA

## loop to generate null communities and calculate expected log leaf area trait ranges
for(ii in 1:reps){
  
  sample(species_pool$logLeafArea, Np28)->lla_random_community_p28
  
  lla_null_range_p28[ii]<-abs(max(lla_random_community_p28)-min(lla_random_community_p28))
  
}

hist(lla_null_range_p28, xlab= "range of log leaf area", ylab = "distribution from null model, N=7", main= "Histogram of null model range of log leaf area for plot 28", xlim = c(0,4))
# quick *estimate* of sig. thresholds for plotting (for a two-tailed test)
quantile(lla_null_range_p28, probs=c(0.025, .975))->cutoffs_p28
abline(v=cutoffs_p28, lty=2)
text("2.5%", x=cutoffs_p28[1], y=180, pos=4)
text("97.5%", x=cutoffs_p28[2], y=180, pos=4)

## expected value (mean) from null:
abline(v=mean(lla_null_range_p28), lty=2)
text("mean", x=mean(lla_null_range_p28), y=265, pos=4)
...
text("observed value", x=lla_obs_range_p28, y= 250, pos=4, col="red")
